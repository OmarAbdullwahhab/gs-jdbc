package gs.jdbc;

import java.sql.Connection;
import java.sql.DriverManager;
import java.sql.SQLException;

//Driver..
//impl jdbc api...
public class Main1 {

    public static void main(String[] args) throws SQLException {
        //Connection..                                          "jdbc:h2:tcp://localhost:9092/./gs-jdbc";
        //Connection connection = DriverManager.getConnection(H2DatabaseServerUtil.DB_URL,"sa","123");
        //System.out.println("Connection 2 to h2 done....");
        //h2-console.
        //H2WebUtil console = new H2WebUtil();
        //console.startWebConsole();

        //Client Server --> oracle/micorsoft/mysql/mariadb/db2/ (sybase). --> tcp
        //Embedded Database.. (file ) // user--> pc1, user-->pc1.

        //SQL Server MS. --> SQL Developer --> Workbench MYSQL
    }
}
